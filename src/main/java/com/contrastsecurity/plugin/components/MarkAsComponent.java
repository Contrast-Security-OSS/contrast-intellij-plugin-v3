...
public class MarkAsComponent extends JBPanel {
    ...
    private void configureMarkAsContainer() {
        ...
        if (StringUtils.isNotEmpty(status)) {
            ...
            if (StringUtils.equalsIgnoreCase(status, Constants.CHECKBOXES.NOT_A_PROBLEM)) {
                Object subStatus =
                        cacheDataService.get(traceID + "-" + Constants.UTILS.MARK_AS_SUB_STATUS);
        ...
        markAsComboBox.addActionListener(
                e -> {
                    ...
                    reasonComboBox.setEnabled(isNotAProblem);

                    if (isNotAProblem && StringUtils.isBlank(commentArea.getText())) {
                        applyPlaceholder();
                    }

                    if (!isNotAProblem && isPlaceholderVisible()) {
                        clearPlaceholder();
                    }
    ...
    private void configureCommentContainer() {
        ...
        if (isNotAProblem && StringUtils.isBlank(commentArea.getText())) {
            commentArea.setText(
                    localizationUtil.getMessage(Constants.MESSAGES.NOT_A_PROBLEM_STATUS));
            commentArea.setForeground(JBColor.GRAY);
        } else if (StringUtils.isBlank(commentArea.getText())) {
            commentArea.setText(StringUtils.EMPTY);
            commentArea.setForeground(
                    JBColor.isBright() ? Color.BLACK : Color.WHITE);
        }

        commentArea.setPreferredSize(new Dimension(360, 150));
        commentArea.setOpaque(true);

        JBScrollPane commentScrollPane = new JBScrollPane(commentArea);
        commentScrollPane.setPreferredSize(new Dimension(360, 150));

        commentContainer.add(commentScrollPane);
    ...
    private void updateOkButtonState() {
        ...
        boolean hasValidComment =
                StringUtils.isNotBlank(commentArea.getText()) && !isPlaceholderVisible();

        okButton.setEnabled(!isNotAProblem || hasValidComment);
    }


    private boolean isPlaceholderVisible() {
        return localizationUtil.getMessage(Constants.MESSAGES.NOT_A_PROBLEM_STATUS).equals(commentArea.getText());
    }

    private void applyPlaceholder() {
        commentArea.setText(localizationUtil.getMessage(Constants.MESSAGES.NOT_A_PROBLEM_STATUS));
        commentArea.setForeground(JBColor.GRAY);
    }

    private void clearPlaceholder() {
        commentArea.setText(StringUtils.EMPTY);
        commentArea.setForeground(JBColor.foreground());
    }

    private void registerCommentFocusListener() {
    ...
    private void registerCommentListener() {
